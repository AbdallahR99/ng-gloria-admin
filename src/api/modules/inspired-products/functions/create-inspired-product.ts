...
export async function createInspiredProduct(
  ...
  const inspiredProductForCreate = {
    name_en: nameEn,
    name_ar: nameAr,
  ...
  const { data, error } = await supabase
    ...
    .insert(inspiredProductForCreate)
    .select('*')
    .single();
  ...
  if (!data) {
    throw new Error('Failed to create inspired product');
  }
